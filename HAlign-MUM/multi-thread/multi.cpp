...
int my_mul_main(int filenum,int chr)
{
    ThreadPool* thread2 = new ThreadPool(8);
    char* path = arguments::MAF_path;
    std::string ref = "";
    int v = 1;
    char cmd[200];
    int file_i = 1;
    ...
    {
        {
            std::lock_guard<std::mutex> lock1(threadPool1->mutex_fp); // 在申请file-number时加锁
    ...
    //切割 所有位点，然后排序
    Stream::cut_maf(maf_array[0], ans_name, ref);
    //合并
    Stream::sort_hebing(ans_name, ref);
    delete thread2;
    //释放内存
    free(maf_array);
    return 0;
