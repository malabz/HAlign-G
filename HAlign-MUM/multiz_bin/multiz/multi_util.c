...
int print_part_ali_col(struct mafAli* ali, int cbeg, int cend, FILE* fp, struct mafAli*** t) {
    struct mafAli* nali;
    nali = make_part_ali_col(ali, cbeg, cend);
    if (nali != NULL && (row2 == 0 || nali->components->next != NULL))
            //mafWrite(fp, nali);//ztzt
        {
            (**t)->next = nali; (**t) = nali;
        }
    else
...
int my_print_part_ali_col(struct mafAli* ali, int cbeg, int cend, FILE* fp) {
    struct mafAli* nali;
    nali = make_part_ali_col(ali, cbeg, cend);
    if (nali != NULL && (row2 == 0 || nali->components->next != NULL))
        //mafWrite(fp, nali);//ztzt
...
int my_print_part_ali_col_(struct mafAli* ali, int cbeg, int cend, FILE* fp,int t1, int t2, int t3) {
    struct mafAli* nali;
    nali = make_part_ali_col(ali, cbeg, cend);
    if (nali != NULL && (row2 == 0 || nali->components->next != NULL))
        //mafWrite(fp, nali);//ztzt
    {
        my_mafWrite(fp, nali,t1,t2,t3,0);
        mafAliFree(&nali);
    }
    else
        mafAliFree(&nali);
