...
/*
void utils::read_to_pseudo(std::istream& is, std::ofstream& nf, std::vector<std::string>& name, std::vector<size_t>& Length, std::ofstream& nof, std::ofstream& idf) //读入数据
{
    ...
    for (bool flag = false; std::getline(is, each_line); )
    {
        ...
        if (each_line[0] == '>')
        {
            ...
            if (flag)
            {
                ...
                {
                    if (each_sequence[j] == 'N' || each_sequence[j] == 'n')
                    {
                        if (tag)//首个N
                            tag = false;
                        num++;
                ...
                //idf << name.back().size() + 3 + index<<"\n";
                if (num != 0)
                    nf << index << " " << num << " ";
                //n_gap.push_back(utils::Insertion({ index, num }));

            //for (int j = 0; j < n_gap.size(); j++)
               // nf << n_gap[j].index << " " << n_gap[j].number << " ";
                nf << "0 0\n";
...
void utils::write_N_tmp(std::vector<std::vector<utils::Insertion>>& N_gap)
{
    std::ofstream nf(arguments::N_file_name, std::ios::binary | std::ios::out); //判断输出noN路径合法否
    if (!nf)
    {
    ...
    {
        nf << ">" << i << " " << N_gap[i].size() << "\n";
        for (int j = 0; j < N_gap[i].size(); j++)
        {
            nf << N_gap[i][j].index << " " << N_gap[i][j].number << "\n";
        }
    }
    nf.close();
...
void utils::read_N_tmp(std::istream& is, std::vector<utils::Insertion>& n_gap)
{
    size_t index, num;
    while (1)
    {
...
void utils::read_N_tmp(std::istream& is, std::vector<utils::Insertion>& n_gap, int i)
{
...
int utils::get_Jaccard_Bloom(std::string& seq_center, bloom_filter** bloom1, bloom_filter** bloom0)
{
    unsigned long long int _max_len = seq_center.size();
    unsigned long long int len0 = _max_len;
    int K;
    //if (len < 200)
    //    动态规划
    if (_max_len < 10000)//200-10000
        K = 5 + _max_len / 1000;
    else
    {
        unsigned long long int x = 1024;
        unsigned long long int Len = _max_len * 10000;
        int k = 5;
        while (x < Len)
        {
            x *= 4;
    ...
    if (_max_len <= 10)
        ...
        K = 3;
    else if (_max_len <= 100)
        K = 4;
    else if (_max_len <= 200)
        K = 5;
    unsigned char* seq_char = reinterpret_cast<unsigned char*>(const_cast<char*>(seq_center.c_str()));

    const uint32_t seed = 0;
    bloom_parameters parameters;         // Bloom Filter 参数
    parameters.projected_element_count = _max_len / 2;   // 预计定位元素个数为序列 seq0 的一半

    parameters.false_positive_probability = 0.001;    // 误判率为 0.1%
    if (K >= 10)
        parameters.false_positive_probability = 0.01;
    parameters.random_seed = 0xA5A5B6B6;             // 随机数种子
    if (_max_len < 2000)
        parameters.projected_element_count = _max_len;   // 预计定位元素个数为序列 seq0
    else
        len0 = len0 / 2;
    parameters.compute_optimal_parameters();         // 计算出最优参数
    *bloom1 = new bloom_filter(parameters);       // 创建 Bloom Filter
    *bloom0 = new bloom_filter(parameters);       // 创建 Bloom Filter


    uint32_t tmp_hash;
    for (size_t i = 0; i < len0 - K + 1; ++i) {
        tmp_hash = 0;
        MurmurHash3_x86_32(seq_char + i, K, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        (*bloom1)->insert(tmp_hash);    // 将哈希值插入 Bloom Filter 中
    }
    for (int zt = 0; zt < _max_len; zt++)
        seq_char[zt] = 3 - seq_char[zt];
    std::reverse(seq_char, seq_char + _max_len);
    for (size_t i = 0; i < len0 - K + 1; ++i) {
        tmp_hash = 0;
        MurmurHash3_x86_32(seq_char + i, K, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        (*bloom0)->insert(tmp_hash);    // 将哈希值插入 Bloom Filter 中
    }
...
bool utils::JaccardSimilarity(bloom_filter* bloom1, bloom_filter* bloom0, std::vector<unsigned char>& vec, int k)
{
    unsigned char* seq_char = vec.data();
    size_t len;
    const uint32_t seed = 0;
    if (k <= 6)
        len = vec.size();
    ...
    for (size_t i = 0; i < len - k + 1; ++i) {
        tmp_hash = 0;
        MurmurHash3_x86_32(seq_char + i, k, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        if (bloom1->contains(tmp_hash)) {  // 如果 Bloom Filter 中存在 tmp_hash，则说明存在相同子序列
            count1++;
        }
        if (bloom0->contains(tmp_hash)) {  // 如果 Bloom Filter 中存在 tmp_hash，则说明存在相同子序列
            count0++;
        }

    }

    //std::cout << count1 + 2 * k - count0 << " " << count1 + 2 * k << " " << count0 << "\t\t\t" << count1 + 2 * k - count0 << "   c1\n";
    return count1 + 2 * k >= count0;
}

bool utils::JaccardSimilarity2(int count1, int count0, unsigned char* seq_center, unsigned char* seq_center_back, unsigned char* seq_i, size_t len_center, size_t len_i, std::vector<unsigned char>& vec, int k)
{

    vec.insert(vec.begin(), 0);
    unsigned char* seq1 = utils::BLAST_PackDNA(vec);
    vec.insert(vec.begin(), 0);
    unsigned char* seq2 = utils::BLAST_PackDNA(vec);
    vec.insert(vec.begin(), 0);
    unsigned char* seq3 = utils::BLAST_PackDNA(vec);
    vec.erase(vec.begin()); vec.erase(vec.begin()); vec.erase(vec.begin());

    size_t len1 = len_center / 2;
    size_t len0 = len_i / 2;

    //const int k = 10;                     // 设置子串长度为 30（可根据实际情况调整）
    const uint32_t seed = 0;
    bloom_parameters parameters;         // Bloom Filter 参数
    ...
    if (len_center < 200)
    {
        len1 *= 2;
        len0 *= 2;
        parameters.projected_element_count = len0;   // 预计定位元素个数为序列 seq0
        k = 1;
    }
    else if (len_center < 2000)
    {
        parameters.projected_element_count = len0;   // 预计定位元素个数为序列 seq0
        k = 2;
    }
    ...
    bloom_filter bloom1(parameters);
    bloom_filter bloom2(parameters);
    bloom_filter bloom3(parameters);
    uint32_t tmp_hash;
    ...
    for (size_t i = 0; i < len0 - k + 1; ++i) {
        ...
        tmp_hash = 0;
        MurmurHash3_x86_32(seq2 + i, k, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        bloom2.insert(tmp_hash);    // 将哈希值插入 Bloom Filter 中

        tmp_hash = 0;
        MurmurHash3_x86_32(seq3 + i, k, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        bloom3.insert(tmp_hash);    // 将哈希值插入 Bloom Filter 中
    }

    int count11 = 0, count12 = 0, count13 = 0;
    int count01 = 0, count02 = 0, count03 = 0;
    for (size_t i = 0; i < len1 - k + 1; ++i) {
        tmp_hash = 0;
        MurmurHash3_x86_32(seq_center + i, k, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        if (bloom1.contains(tmp_hash)) {  // 如果 Bloom Filter 中存在 tmp_hash，则说明存在相同子序列
            count11++;
        }
        tmp_hash = 0;
        MurmurHash3_x86_32(seq_center_back + i, k, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        if (bloom1.contains(tmp_hash)) {  // 如果 Bloom Filter 中存在 tmp_hash，则说明存在相同子序列
            count01++;
        }
        ...
        tmp_hash = 0;
        MurmurHash3_x86_32(seq_center + i, k, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        if (bloom2.contains(tmp_hash)) {  // 如果 Bloom Filter 中存在 tmp_hash，则说明存在相同子序列
            count12++;
        }
        tmp_hash = 0;
        MurmurHash3_x86_32(seq_center_back + i, k, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        ...
        tmp_hash = 0;
        MurmurHash3_x86_32(seq_center + i, k, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
        if (bloom3.contains(tmp_hash)) {  // 如果 Bloom Filter 中存在 tmp_hash，则说明存在相同子序列
            count13++;
        }
        tmp_hash = 0;
        MurmurHash3_x86_32(seq_center_back + i, k, seed, &tmp_hash); // 使用 MurmurHash 进行哈希计算，并将哈希值保存至 hashes0[i] 中
    ...
    int max_count1 = std::max({ count1, count11, count12, count13 });
    int max_count0 = std::max({ count0, count01, count02, count03 });

    std::cout << max_count1 - max_count0 << " " << max_count1 << " " << max_count0 << "   c2\n";
    delete[] seq1;
    delete[] seq2;
    delete[] seq3;
    return max_count1 + 4 * k >= max_count0;
    //return (CaculateSimilarity(seq_center, seq_i, len_center, len_i) >= CaculateSimilarity(seq_center_back, seq_i, len_center, len_i));
}
...
//按拉长fasta输出,按内存原串插入 ##################################################   2fasta   ##########
size_t utils::insert_and_write_fasta(std::ostream& os, std::vector<std::vector<Insertion>>& insertions,
    std::vector<std::vector<Insertion>>& N_insertions, std::vector<std::string>& name, std::vector<bool>& sign, std::vector<size_t>& Length, bool mg_tag)
    //写回比对结果
{
    ...
    //std::cout << "N_insertions " << sequence_number << " " << N_insertions[0].size() << " " << N_insertions[1].size() <<"\n";
...
//*************insert_begin***********
    ...
    for (int k = 0; k < sequence_number; k++)
    {
        ...
        read_N_tmp(ns, N_insertions[k]);
        if (!sign[k])
        {
            for (int p = 0; p < N_insertions[k].size(); p++)//N_insertion 改变
            {
                N_insertions[k][p].index = Length[k] - N_insertions[k][p].index;
                ALength[k] += N_insertions[k][p].number;
            }
            std::reverse(N_insertions[k].begin(), N_insertions[k].end());
        }
        else
        {
            for (int p = 0; p < N_insertions[k].size(); p++)//N_insertion 改变
        ...
        std::vector<Insertion>().swap(insertions[k]);
        std::vector<Insertion>().swap(N_insertions[k]);
    ...
    for (i = 0; i < sequence_number; i++)
    {
        ...
        if (i == 0)
        {
            ...
            //std::cout << tmp_vector.size() << " tmp_vector.size()\n";
        }
        ...
        //std::cout << cur_seq.size() <<" "<<k<<" "<<ti << " cur_seq.size() k ti\n";
        while (k < cur_seq.size())tmp_vector[ti++] = cur_seq[k++];
        // std::cout << cur_seq.size() << " " << k << " " << ti << " !!!cur_seq.size() k ti\n";
        cur_seq = "";
        all_seq.resize(all_size);
        mi = 0;
        k = 0;
        ti = 0;
        for (j = 0; j < more_insertions[i].size(); j++)
        ...
        //std::cout << more << " " << k << " " << ti << " more k ti\n";

        while (ti < more) all_seq[k++] = tmp_vector[ti++];
        //std::cout << more << " " << k << " " << ti << " !!!more k ti\n";
        os << "> " << name[i];
        if (sign[i]) os << "(+)\n";
        else os << "(-)\n";
        //std::cout << all_seq.size() << "  all_seq.size()\n";
        if (mg_tag) //maf输出两个文件
        {
            std::ofstream osi(arguments::out_file_name + "/fasta/" + std::to_string(i) + ".fasta", std::ios::binary | std::ios::out);
            for (k = 0; k < all_seq.size(); k++)
            {
                os << all_seq[k];
                osi << all_seq[k];
            }
        ...
        {
            for (k = 0; k < all_seq.size(); k++) os << all_seq[k];
        }
        os << "\n";
        std::string().swap(all_seq);
    }
    ...
    Length = ALength;
    std::vector<size_t>().swap(ALength);
    return all_size;
}
