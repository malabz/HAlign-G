...
std::tuple<insert, insert>
    Kband::mum_PSA_AGP_Kband3(const std::vector<unsigned char>& sequence1, int a_begin, int a_end, const std::vector<unsigned char>& sequence2, int b_begin, int b_end, int cmatch, int cmismatch, int cd, int ce)
{
    ...
    std::vector<unsigned char> A(sequence1.begin() + a_begin, sequence1.begin() + a_end);
    std::vector<unsigned char> B(sequence2.begin() + b_begin, sequence2.begin() + b_end);
...
std::tuple<insert, insert>
    BWT_MINI::mum_BWT_mini(Kband* kband, const std::vector<unsigned char>& sequence1, int a_begin, int a_end, const std::vector<unsigned char>& sequence2, int b_begin, int b_end,
        int thresh, int d, int e)
{
    ...
    {
        return kband->mum_PSA_AGP_Kband3(sequence1, a_begin, a_end, sequence2, b_begin, b_end);
    }
    else if (b_len <= a_len)  //保持A长
    {
        if ((a_len / b_len > 100) && (b_len < 1000)) //离谱差异，插空补长
        {
        ...
        std::vector<unsigned char> veci(sequence2.begin() + b_begin, sequence2.begin() + b_end);
        ...
        for (size_t j = 0; j != intervals.size(); ++j)
        {
            ...
            auto [lhs_gaps, rhs_gaps] = mum_BWT_mini(kband, sequence1, centre_begin, centre_end, sequence2, sequence_begin, sequence_end, thresh); //分治到比thresh小，然后k-band
    ...
    {
        if ((b_len / a_len > 100) && (a_len < 1000)) //离谱差异，插空补长
        {
        ...
        std::vector<unsigned char> veci(sequence1.begin() + a_begin, sequence1.begin() + a_end);
        std::vector<triple> common_substrings = _optimal_path(mini_bwt.get_common_substrings(veci.cbegin(), veci.cend(), 5, false)); //common_substrings [A.index,B.index,length]
        if (common_substrings.size() == 0)
            common_substrings = _optimal_path(mini_bwt.get_common_substrings(veci.cbegin(), veci.cend(), 1, false));
        std::vector<unsigned char>().swap(veci);
        ...
        for (size_t j = 0; j != intervals.size(); ++j)
        {
            ...
            auto [lhs_gaps, rhs_gaps] = mum_BWT_mini(kband, sequence1, sequence_begin, sequence_end, sequence2, centre_begin, centre_end, thresh); //分治到比thresh小，然后k-band
            a_gap.insert(a_gap.end(), lhs_gaps.begin(), lhs_gaps.end());
            b_gap.insert(b_gap.end(), rhs_gaps.begin(), rhs_gaps.end());
            insert().swap(lhs_gaps);
            insert().swap(rhs_gaps);
    ...
    int thresh, int d, int e)
{
    ...
    else if (a_len > b_len)
    {
        if ((a_len / b_len > 100) && (b_len < 1000)) //离谱差异，插空补长
        {
            b_gaps.push_back(in(b_begin, a_len - b_len));
            return std::make_tuple(std::move(a_gaps), std::move(b_gaps));
        }
    }
    else if (a_len < b_len)
    {
        if ((b_len / a_len > 100) && (a_len < 1000)) //离谱差异，插空补长
        {
            a_gaps.push_back(in(a_begin, b_len - a_len));
            return std::make_tuple(std::move(a_gaps), std::move(b_gaps));
        }
    }
    
    std::vector<unsigned char> seqA_all = sequence1->subvec(a_begin, a_end);
    std::vector<unsigned char> seqB_all = sequence2->subvec(b_begin, b_end);
   
    std::tuple<insert, insert> tmp;
    std::vector<utils::Insertion> AN, BN;

    std::vector<unsigned char> seqA, seqB;
    seqA.reserve(a_end - a_begin);//预留大小
    seqB.reserve(b_end - b_begin);
    //A，B 的N信息********
    size_t iindex = 0, nnum=0;
    for (auto& c : seqA_all)
    {
        switch (c)
        {
    ...
    std::vector<unsigned char>().swap(seqA_all);
    std::vector<unsigned char>().swap(seqB_all);

    //std::cout << seqA.size() << "\t" << seqB.size() << "   alen-blen\n";
    //比对，得到insert-gap
    if ((a_len < thresh) && (b_len < thresh)) //A，B都小于阈值
        tmp = kband->mum_PSA_AGP_Kband3(seqA, 0, seqA.size(), seqB, 0, seqB.size());
    else
        tmp = BWT_MINI::mum_BWT_mini(kband, seqA, 0, seqA.size(), seqB, 0, seqB.size(), thresh);
    
    utils::insertion_gap_out_new_mum(seqA, seqB, tmp, AN, BN, a_begin, b_begin); //MINI bwt变换求
    return tmp;
}
