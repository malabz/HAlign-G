...
struct mafAli* trans_ref(char* ref, struct mafAli* a) {
    struct mafComp* c,*pre,* temp;
    int tag = -1;
    int row = 0;
    for (c = a->components; c != NULL; c = c->next) {
        if (strcmp(c->src, ref) == 0)
            tag = row;
        row++;
    }
    if (tag == 0)
    ...
    //ref
    pre = a->components;
    for (c = a->components->next; c != NULL;) {
        if (strcmp(c->src, ref) == 0)
            break;
        pre = c;
        c = c->next;
...
void mafWrite_split(FILE* f, struct mafAli* a) {
    struct mafComp* c;
    struct mafComp* head;
    int srcChars = 0, startChars = 0, sizeChars = 0, srcSizeChars = 0, row=0;
    char src[500], name[200], chr[200];
    for (c = a->components; c != NULL; c = c->next) {
        srcChars = MAX(srcChars, (signed int)strlen(c->src));
        startChars = MAX(startChars, digitsBaseTen(c->start));
    ...
    {
        head = a->components;
        for (c = a->components->next; c != NULL; c = c->next) {
            fprintf(f, "a score=0\n");
            fprintf(f, "s %-*s %*d %*d %c %*d %s\n",
                srcChars, head->src, startChars, head->start, sizeChars, head->size,
                head->strand, srcSizeChars, head->srcSize, head->text);
            fprintf(f, "s %-*s %*d %*d %c %*d %s\n",
