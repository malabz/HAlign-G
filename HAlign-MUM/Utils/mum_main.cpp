...
void BinaryMUMSave(int I, std::vector<std::string>& files, int inThreads, int filer)
{
	// 构造参数字符串
	std::string arg0;
	arg0 = "mynuc";
	...
	std::string arg7 = std::to_string(filer);
	// 创建字符指针数组
...
void BinaryMUMLoad(int I, std::vector<std::string>& files, int inThreads, int filer)
{
	// 构造参数字符串
	std::string arg0;
	arg0 = "mynuc";
...
void mum_main_psa(int I, std::vector<std::string>& files, std::vector<std::vector<size_t>>& Length_All, std::vector<std::vector<std::string>>& Name_All, int inThreads, bool tag_load, int filer)
{
	...
	if ((!tag_load) && (I == 1))
		BinaryMUMSave(I, files, inThreads, filer);
	else
		BinaryMUMLoad(I, files, inThreads, filer);
	...
	for (int Fi = 0; Fi < NameA.size(); Fi++)
	{
		...
		for (auto& structi : Structs)
		{
			...
			//NaA.erase(std::remove(NaA.begin(), NaA.end(), ' '), NaA.end());
			//NaA = NaA.substr(0, NaA.find(' '));

			NaB = NameB[Bi];
			//NaB.erase(std::remove(NaB.begin(), NaB.end(), '.'), NaB.end());
			//NaB.erase(std::remove(NaB.begin(), NaB.end(), ' '), NaB.end());
			//NaB = NaB.substr(0, NaB.find(' '));
...
void mum_main(std::vector<std::string>& files, std::vector<std::vector<size_t>>& Length_All, std::vector<std::vector<std::string>>& Name_All, int inThreads, bool tag_load, int filer) {
	if (tag_load)
	{
		for (int I = 1; I < files.size(); I++)
		{
			threadPool0->execute(&mum_main_psa, I, std::ref(files), std::ref(Length_All), std::ref(Name_All), inThreads, tag_load, filer);
		}
	}
	else
	{
		mum_main_psa(1, std::ref(files), std::ref(Length_All), std::ref(Name_All), arguments::seq_num, tag_load, filer);
