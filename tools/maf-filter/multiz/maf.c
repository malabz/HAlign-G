...
void mafRead_filter_writeAll(char* fileName, char* ans_name, int MAFthresh1, int MAFthresh2, int MAFthresh3) {
    struct mafFile* mf = mafOpen(fileName, 1);
    struct mafAli* a = NULL, * last = NULL, * tail = NULL;
    ...
    while (a)
    {
        my_mafWrite(fpw1, a, MAFthresh1, MAFthresh2, MAFthresh3, 0);
        last = a->next;
        mafAliFree(&a);
        a = last;
    }
...
void mafRead_filter_writeAll2(char* fileName, char* ans_name, char* seq_name1, char* seq_name2, int MAFthresh1, int MAFthresh2, int MAFthresh3) {
    struct mafFile* mf = mafOpen(fileName, 1);
    struct mafAli* a = NULL, * last = NULL, * tail = NULL;
    ...
    while (a)
    {
        my_mafWrite2(fpw1, a, seq_name1, seq_name2, MAFthresh1, MAFthresh2, MAFthresh3, 0);
        last = a->next;
        mafAliFree(&a);
        a = last;
    }
...
void my_mafWrite2(FILE* f, struct mafAli* a, char* seq_name1, char* seq_name2, int thresh1, int thresh2, int thresh3, int thresh4) //1-len,2-num,3-Continuity,4-score.
{
    thresh4 = 0;
    if (a->score <= thresh4 || a->textSize < thresh1)
        return;
    ...
    char* text;
    for (c = a->components; c != NULL; c = c->next) {
        if (strcmp(c->src, seq_name1) == 0 || strcmp(c->src, seq_name2) == 0);
        else
            continue;
        srcChars = MAX(srcChars, (signed int)strlen(c->src));
        startChars = MAX(startChars, digitsBaseTen(c->start));
