...
int mafAlicompare(const void* a, const void* b) {
    struct mafAli* nodeA = *(struct mafAli**)a;
    struct mafAli* nodeB = *(struct mafAli**)b;
    return nodeA->components->start - nodeB->components->start;
}
...
struct mafAli* mergeSort(struct mafAli* head) {
    ...
    // 创建节点数组
    struct mafAli** nodes = (struct mafAli**)malloc(length * sizeof(struct mafAli*));
    temp = head;
    ...
    // 排序节点数组
    qsort(nodes, length, sizeof(struct mafAli*), mafAlicompare);
...
void my_mafReadAll_p(struct mafFile* mf, struct mafAli** maf_arrayi) {
    ...
    for (last = NULL; (a = my_mafNext_noY(mf)) != NULL; last = a)
        if (last == NULL)
            mf->alignments = a;
        else
            last->next = a;
    (*maf_arrayi) = mf->alignments;
    mf->alignments = NULL;
    mafFileFree(&mf);
}
