...
void Stream::sort_hebing(std::string sourceName, std::string fileName, std::string ref_name)
{
    struct mafFile* maf;
    struct mafAli* cp_list, * tmp_list = NULL, * now, * next, * tail = NULL, * tmp = NULL;
    //init_scores70();

    struct mafFile* filefp = mafOpen(const_cast<char*>(sourceName.c_str()), 1);
    cp_list = my_mafNext(filefp);
...
void sortAndSaveChunks(char* fileName, const char* dirName, size_t chunkSize, int* chunkCount) {
    struct mafFile* maf = mafOpen(fileName, 1);
    struct mafAli* chunkHead = NULL;
    struct mafAli* chunkTail = NULL;
    struct mafAli* a;
    size_t chunkCounter = 0;
    size_t totalCount = 0;
    char filename[256];

    while ((a = my_mafNext(maf)) != NULL) {
        if (chunkCounter == 0) {
            chunkHead = a;
            chunkTail = a;
        }
        ...
        if (chunkCounter >= chunkSize) {
            chunkTail->next = NULL;
            chunkHead = mergeSort(chunkHead);
            sprintf(filename, "%stemp_chunk_%d.dat", dirName, (*chunkCount)++);
            writeSortedChunk(chunkHead, filename);
            chunkCounter = 0;
        }
    }
...
void mergeChunks(int chunkCount, const char* dirName, char* outputFile) {
    struct mafFile** files = (struct mafFile**)malloc(chunkCount * sizeof(struct mafFile*));
    struct mafAli** currentElements = (struct mafAli**)malloc(chunkCount * sizeof(struct mafAli*));
    char filename[256];
    // 打开所有临时文件
    for (int i = 0; i < chunkCount; i++) {
        files[i] = NULL;
        currentElements[i] = NULL;
        sprintf(filename, "%stemp_chunk_%d.dat", dirName, i);
        files[i] = mafOpen(filename, 1);
        if (files[i] != NULL)
            currentElements[i] = my_mafNext(files[i]);
    ...
    fprintf(outFile, "##maf version=1 scoring=N/A\n");
    while (1) {
        // 找到最小的元素
        int minIndex = -1;
        struct mafAli* minElement = NULL;
        for (int i = 0; i < chunkCount; i++) {
            if (currentElements[i] != NULL) {
                if (minIndex == -1 || (currentElements[i]->components->start < minElement->components->start)) {
                    minIndex = i;
                    minElement = currentElements[i];
                }
            }
        }
        if (minIndex == -1) {
        ...
        // 写出当前最小的元素
        mafWrite(outFile, minElement);
        mafAliFree(&minElement);

        currentElements[minIndex] = NULL;
        // 读取下一个元素
        if (files[minIndex] != NULL)
            currentElements[minIndex] = my_mafNext(files[minIndex]);
        if (currentElements[minIndex] == NULL)
        {
            mafFileFree(&(files[minIndex]));
            files[minIndex] = NULL;
        }
    }
    // 清理
...
void sort_maf_file(char* filepath, int chunkSize)
{
    std::string dirpath = filepath;
    dirpath = dirpath + "_/";
    if (my_mk_dir(dirpath) != 0)
    {
        std::cout << dirpath << "; Folder creation error\n";
        exit(-1);
    }
  //  std::cout << "111  " << getPeakRSS() / 1024 << " kB\n";//输出内存耗费
    int chunkCount = 0;
 //   std::cout << "222  " << getPeakRSS() / 1024 << " kB\n";//输出内存耗费
    sortAndSaveChunks(filepath, dirpath.c_str(), chunkSize, &chunkCount);
 //   std::cout << "333  " << getPeakRSS() / 1024 << " kB\n";//输出内存耗费
    mergeChunks(chunkCount, dirpath.c_str(), filepath);
 //   std::cout << "444  " << getPeakRSS() / 1024 << " kB\n";//输出内存耗费
